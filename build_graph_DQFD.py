...
def build_train(q_func, ob_space, ac_space, optimizer, sess, grad_norm_clipping=None,
    ...
    with tf.variable_scope("loss", reuse=reuse):
        ...
        is_demos = tf.placeholder(tf.float32, [None], name="demos")
        ...
        margin_l = 0.8 * (tf.ones_like(one_hot_actions, dtype=tf.float32) - one_hot_actions)
        margin_masked = tf.reduce_max(step_model.q_values + margin_l, 1)
        
        loss_E = 1  * (margin_masked - q_t_selected) * is_demos

        # ==========L2 loss=========
        loss_l2 = (1e-5) * tf.reduce_sum([tf.reduce_sum(tf.square(variables)) for variables in q_func_vars])

        
        # compute the error (potentially clipped)
        td_error = q_t_selected - tf.stop_gradient(q_t_selected_target)
        errors = tf_util.huber_loss(td_error)
        all_loss = errors + loss_E
        weighted_error = tf.reduce_mean(importance_weights_ph * all_loss)+loss_l2

        tf.summary.scalar("td_error", tf.reduce_mean(td_error))
        tf.summary.scalar("loss_Expert",tf.reduce_mean(loss_E))
        tf.summary.scalar("loss", weighted_error)
        tf.summary.scalar("l2_loss", loss_l2)
