...
def main():
  ...
  def dummy_expert(_obs):
    ...
    #print(action)
    return acc
  #model = SAC.load("sacfd_carla.zip",env)
  #dataset = ExpertDataset(expert_path='expert_carla_new_human_continuous_test_key.npz')
  #model = SAC(LnCnnPolicy,env,gamma=0.995,n_step=True)
  ...
  #generate_expert_traj(dummy_expert, 'expert_carla_new_human_test_discrete_3',env,n_timesteps=0,n_episodes=1,limit_return=200,mlp_obs=True)
  #model = SAC.load("./dqn_logs_xunhuan_sac2/rl_model_30000_steps.zip",env)
  #model = DQN.load("rl_model_90000_steps.zip")
  #generate_expert_traj(model, 'expert_carla_sacfdrule_test_2',env,n_timesteps=0,n_episodes=50,limit_return=200,mlp_obs=True)
  # path = 'C:/Users/24829/Desktop/gym-carla-master/'
  # dataset = ExpertDataset(expert_path=path+'expert_carla_dqnrule_test_2.npz')
  # print(len(np.where(dataset.dones==1)[0]))
  # print(len(np.where(dataset.dones==2)[0]))
  # print(len(np.where(dataset.dones==3)[0]))
  # print("expert saved!")
  '''
  n_actions = env.action_space.shape[-1]
  model = TD3(CnnPolicy,env,gamma=0.995,le.npzarning_starts=10000,batch_size=64,gradient_steps=1,train_freq=1,action_noise= NormalActionNoise(mean=np.zeros(n_actions), sigma=0.1 * np.ones(n_actions)))
  ...
  '''
  #dataset = ExpertDataset(expert_path='C:/Users/24829/Desktop/gym-carla-master/expert_carla_sac_mlp.npz',traj_limitation=100)
  #dataset.plot()
  ...
  dataset = ExpertDataset(expert_path='C:/Users/24829/Desktop/gym-carla-master/expert_carla_sacfdrule_test_2.npz',traj_limitation=100)
  print(len(np.where(dataset.dones==1)[0]))
  print(len(np.where(dataset.dones==2)[0]))
  print(len(np.where(dataset.dones==3)[0]))
  '''
  j = 0
  for i in range(len(dataset.dones)):
    if dataset.dones[i]==2:
      j+=1
  print(j)
  '''
  #dataset.plot()
  #print("mean_exp_return is:",np.mean(dataset.returns))
  '''
  #model = GAIL(MlpPolicy,env, dataset, verbose=1,gamma=0.995,max_kl=0.01,lam=0.95,vf_iters=2,hidden_size_adversary=128,timesteps_per_batch=512,g_step=2)
  
  # Note: in practice, you need to train for 1M steps to have a working policy
  checkpoint_callback = CheckpointCallback(save_freq=1024*8, save_path='./dqn_logs_xunhuan_GAIL/')
  model.pretrain(dataset,n_epochs=200,val_interval=1)
  model.save("BC_gail_CARLA_2")
  #model = GAIL.load("BC_gail_CARLA.zip",env)
  model.tensorboard_log = './tensorboard_dqn_log/GAIL/'
  model.learn(total_timesteps=100000,callback=checkpoint_callback)
  model.save("gail_CARLA")
  
  model = GAIL.load("BC_gail_CARLA")
  obs = env.reset()
  while True:
